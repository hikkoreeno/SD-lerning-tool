...
export default function Home() {
  ...
  const handleDownloadObsidian = async () => {
    ...
    const baseFileName = `${date}_${briefPrompt}_${timestamp}`;
    const mdFileName = `${baseFileName}.md`;

    // Handle image filename and type
    let imgFileName = "";
    if (image) {
      const match = image.match(/^data:image\/(\w+);base64,/);
      const ext = match ? match[1] : "png";
      imgFileName = `${baseFileName}.${ext}`;
    }
...
## 🖼️ Reference Image
${image ? `![[${imgFileName}]]` : "No image attached."}
...
`;
    ...
    if (savePath) {
      ...
      try {
        const filesToSave = [
          { fileName: mdFileName, content: markdown, isBinary: false }
        ];

        if (image && imgFileName) {
          filesToSave.push({ fileName: imgFileName, content: image, isBinary: true });
        }

        const res = await fetch("/api/save", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ folderPath: savePath, files: filesToSave }),
        });
        const data = await res.json();
        if (data.success) {
          setSaveStatus("success");
          setStatusMessage(`Saved ${filesToSave.length} files to ${savePath}`);
    ...
    const triggerDownload = (content: string, name: string, type: string) => {
    ...
    triggerDownload(markdown, mdFileName, "text/markdown");
